...
impl CartService {
    pub fn get_cart(command: GetCart) -> Result<Cart> {
        let cart = CartRepository::get_cart(&command).map_err(|_| RepositoryError::CartNotFound{ user_id: command.user_id })?;

        Ok(cart)
    }

    pub fn add_to_cart(command: CreateCart) -> Result<()> {
        CartRepository::get_cart(&GetCart{ user_id: command.user_id }).map_err(|_| RepositoryError::CartNotFound{ user_id: command.user_id })?;
    ...
    pub fn update_cart_line(command: UpdateCart) -> Result<()> {
        match CartRepository::get_cart(&GetCart{ user_id: command.cart.user_id }) {
            Ok(_) => {
                CartRepository::update_cart(&command).map_err(|_| RepositoryError::CartDbError)?;
            },
            Err(_) => {
                CartRepository::create_cart(&CreateCart { user_id: command.cart.user_id })
